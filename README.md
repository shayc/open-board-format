...
ESM only — CommonJS (`require`) is not supported. Runs in the browser and in Node.js 22+.
...
One naming convention covers the whole surface: `parse*` takes a JSON string, `validate*` takes an already-parsed object, `load*` takes a browser `File`, `stringify*` returns a JSON string — and `extractOBZ`/`createOBZ` operate on whole archives.
